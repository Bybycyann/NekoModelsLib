...
#旧绑定剔除
execute as @e[tag=nkworkblock] run function nkworkblock:open/unlik with entity @p[tag=nktemp]
#选中方块
function nktool:raycast/0
#导入player-UUID
function nkworkblock:open/binding with entity @s
    #>binding
...
tag @s remove nktemp
#重置成就触发器
advancement revoke @s only nkworkblock:open