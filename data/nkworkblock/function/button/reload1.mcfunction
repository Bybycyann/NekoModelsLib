...
#重置容器界面
execute as @e[type=minecraft:marker,tag=nkworkblock,tag=nktemp] run data modify storage nmo:temp button merge from entity @s data
execute as @e[type=minecraft:marker,tag=nkworkblock,tag=nktemp] at @s align xyz run function nkworkblock:button/reload2 with storage nmo:temp button
#后处理
tag @s remove nktemp
